---
layout: project
title: Monopropellant Actuation
cover_image: /assets/fabrication-integrated-design-lab/singleLegSetup.jpg
description: Wireless control system for a propellant-powered oscillatory robot and hydrogen peroxide reaction chamber.
tags:
  - Research
  - Robotics
  - Propulsion
order: 3
display_dates: June 2024 - August 2024
category: research
---

## Monopropellant Powered Soft Robot

**Collaborators:** S. D. Gollob, A. Comoretto, Q. Yu, K. O. Adebajo, J. T. B. Overvelde, Kaitlyn Becker, Ellen Roche

<div style="display:flex;gap:2em;flex-wrap:wrap;justify-content:center;margin-bottom:2em;align-items:center;">
  <!-- Triangle image layout -->
  <div style="display:flex;align-items:center;">
    <div style="display:flex;flex-direction:column;gap:1.2em;justify-content:center;">
      <div style="text-align:center;">
        <img src="{{ site.baseurl }}/assets/fabrication-integrated-design-lab/singleLegSetup.jpg" alt="Single Leg Setup" style="width:100%;max-width:340px;border-radius:1em;box-shadow:0 2px 12px rgba(0,0,0,0.10);margin-bottom:0.5em;">
        <div style="color:#aaa;font-size:1em;margin-top:0.5em;">Single Leg Robot Setup</div>
      </div>
      <div style="text-align:center;">
        <img src="{{ site.baseurl }}/assets/fabrication-integrated-design-lab/chamberCAD.jpg" alt="Chamber CAD" style="width:100%;max-width:340px;border-radius:1em;box-shadow:0 2px 12px rgba(0,0,0,0.10);margin-bottom:0.5em;">
        <div style="color:#aaa;font-size:1em;margin-top:0.5em;">Hydrogen Peroxide Reaction Chamber CAD</div>
      </div>
    </div>
    <div style="display:flex;align-items:center;justify-content:center;height:100%;margin-left:1.2em;">
      <div style="text-align:center;">
        <img src="{{ site.baseurl }}/assets/fabrication-integrated-design-lab/tubotPic.jpg" alt="Tubot" style="width:100%;max-width:320px;border-radius:1em;box-shadow:0 2px 12px rgba(0,0,0,0.10);background:#23272a;">
        <div style="color:#aaa;font-size:1em;margin-top:0.5em;">Tubot Prototype</div>
      </div>
    </div>
  </div>
  <!-- Paper preview column -->
  <div style="flex:1;min-width:320px;max-width:700px;display:flex;flex-direction:column;align-items:center;">
    <iframe src="{{ site.baseurl }}/assets/fabrication-integrated-design-lab/A_Pressure-Amplifying_Monopropellant_Engine_for_Actuator-Localized_Pneumatic_Power.pdf" width="100%" height="700" style="border-radius:1em;box-shadow:0 4px 24px rgba(0,0,0,0.15);margin-bottom:1em;"></iframe>
    <div style="color:#aaa;font-size:1.05em;margin-bottom:1em;text-align:center;">Preview: Monopropellant Engine Paper (PDF)</div>
  </div>
</div>
<div style="margin:2em 0;text-align:center;">
  <a href="https://ieeexplore.ieee.org/document/11020950" target="_blank" style="display:inline-block;background:#fff;color:#181a1b;padding:0.9em 2em;border-radius:2em;font-weight:700;text-decoration:none;font-size:1.15em;transition:background 0.2s;box-shadow:none;">View Published Paper on IEEE Xplore</a>
</div>

### Key Contributions
- Characterized and developed a wireless control system for a propellant-powered oscillatory robot
- Designed and tested a hydrogen peroxide reaction chamber to prove the energetic benefits of relative pressure systems in feeding this fuel for robotic actuation

### Related Publications & Abstracts

#### Peer-Reviewed Publications
- S. D. Gollob, A. Comoretto, Q. Yu, **M. Fernandez**, K. O. Adebajo, J. T. B. Overvelde, K. Becker, E. Roche. An Actuator-Scale, Pressure-Amplifying, Monopropellant Engine for Untethered Pneumatic Systems (*accepted RoboSoft 2025*).

#### Presentations
- Monopropellant Powered Self-Oscillating Limbs for Untethered Locomotion. *MIT Digital Poster Showcase, November 2024*.
- A Controllable Propellant-Based Power Source for Untethered Soft Robots. *MIT Summer Research Program Poster Showcase, July 2024*. 